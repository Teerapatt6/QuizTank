...
module.exports = {
    ...
    getUserSecurityData: async (userId) => {
        const sql = `SELECT user_id, password_hash, username, email, status, role, xp, profile_pic_url FROM users WHERE user_id = $1`;
        const { rows } = await db.query(sql, [userId]);
        return rows[0];
    },
    ...
    updateProfile: async (userId, payload) => {
        ...
        if (payload.game_audio !== undefined) {
            fields.push(`game_audio = $${paramIdx++}`);
            values.push(payload.game_audio);
        }
        if (payload.game_music !== undefined) {
            fields.push(`game_music = $${paramIdx++}`);
            values.push(payload.game_music);
        }
        if (payload.game_sfx !== undefined) {
            fields.push(`game_sfx = $${paramIdx++}`);
            values.push(payload.game_sfx);
        }
        ...
        const sql = `
            ...
            RETURNING user_id, username, full_name, biography, profile_pic_url, email, game_audio, game_music, game_sfx
        `;
        values.push(userId);
