...
exports.getChallengeById = async (req, res) => {
    try {
        ...
        // Calculate if challenge is active now
        const type = challenge.type_id === 2 ? 'WEEKLY' : 'DAILY';
        const currentDateRef = getUTCDateRef(type);
        const challengeDateRef = challenge.start_date ? new Date(challenge.start_date).toISOString().split('T')[0] : null;

        const isActive = challenge.status === 1 && challengeDateRef === currentDateRef;

        const timeUntilReset = isActive
            ? (challenge.type_id === 2 ? getSecondsUntilWeeklyReset() : getSecondsUntilDailyReset())
            : 0;
