...
exports.register = async (req, res) => {
  ...
  // Username validation: A-Z, a-z, 0-9 only, Min 4, Max 20 characters
  const usernameRegex = /^[a-zA-Z0-9]+$/;
  if (!usernameRegex.test(username)) {
    return res.status(400).json({ error: "Username must consist of letters and numbers only" });
  }
  if (username.length < 4) {
    return res.status(400).json({ error: "Username must be at least 4 characters long" });
  }
  if (username.length > 20) {
    return res.status(400).json({ error: "Username must not exceed 20 characters" });
  }
