        ...
        try {
            ...
            if (payload.questions && payload.questions.length > 0) {
                for (const q of payload.questions) {
                    const qSql = `
                        INSERT INTO questions (game_id, question_type, text, time_limit_seconds, points)
                        VALUES ($1, $2, $3, $4, $5)
                    ...
                    if (q.options && q.options.length > 0) {
                        if (q.question_type === 'MCQ' && (q.options.length < 2 || q.options.length > 6)) {
                            throw new Error(`MCQ Question "${q.text.substring(0, 20)}..." must have 2-6 options.`);
                        }
                        ...
                        const optionPromises = q.options.map((opt, index) => {
                            const optSql = `
                                ...
                                VALUES ($1, $2, $3, $4)
                            `;
                            return client.query(optSql, [qId, opt.text, opt.is_correct || false, index + 1]);
                        });
                        await Promise.all(optionPromises);
                    } else if (q.question_type === 'MCQ') {
                        throw new Error(`MCQ Question "${q.text.substring(0, 20)}..." requires options.`);
                    }

                    // Handle Media
                    if (q.media && q.media.length > 0) {
                        if (q.media.length > 10) {
                            throw new Error(`Question "${q.text.substring(0, 20)}..." has too many media files (Max 10).`);
                        }
