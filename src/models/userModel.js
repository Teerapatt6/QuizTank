...
module.exports = {
    ...
    createUser: async (user) => {
        ...
        const sql = `
            ...
            VALUES ($1, $2, $3, $4, 1, 0)
            RETURNING user_id, username, email, role, level, xp
        `;
        // Default: Level 1, XP 0
        const { rows } = await db.query(sql, [username, email, password_hash, role || 'USER']);
        return rows[0]; 
    },
    ...
    getNavbarInfo: async (userId) => {
        const sql = `
            SELECT user_id, username, profile_pic_url, level, xp, role 
            FROM users 
            WHERE user_id = $1
        `;
    ...
    getUserSecurityData: async (userId) => {
        const sql = `SELECT user_id, password_hash, username, email FROM users WHERE user_id = $1`;
        const { rows } = await db.query(sql, [userId]);
        return rows[0];
    },
