...
export const gameRoomService = {
    ...
    // Get public games
    getPublicGames: async (limit = 20, offset = 0): Promise<GameRoom[]> => {
        const response = await api.get('/public', { params: { limit, offset } });
        return response.data;
    },

    // Get games by username
    getGamesByUsername: async (username: string, limit = 20, offset = 0, sortBy = "newest"): Promise<GameRoom[]> => {
        const response = await api.get(`/user/${username}`, { params: { limit, offset, sortBy } });
        return response.data;
    },

    // Search games
    searchGames: async (query: string, limit = 20, offset = 0, sortBy = "newest"): Promise<GameRoom[]> => {
        const response = await api.get('/search', { params: { q: query, limit, offset, sortBy } });
        return response.data;
    },
    ...
    // Get my favourites
    getMyFavourites: async (limit = 20, offset = 0, sortBy = "newest"): Promise<any[]> => {
        const response = await api.get('/my-favourites', { params: { limit, offset, sortBy } });
        return response.data;
    },
