...
import { Smile, Pencil, Crown, Lock } from "lucide-react";
                ...
                <div className="space-y-4">
                  <div className="flex flex-col sm:flex-row items-center gap-3 md:gap-4">
                    ...
                    <div className="flex-1 text-center sm:text-left min-w-0">
                      <h3 className="text-lg md:text-xl font-bold mb-0.5">{profileName}</h3>
                      <p className="text-muted-foreground text-xs md:text-sm">@{profileUsername}</p>
                      <p className="text-xs md:text-sm mt-1 truncate overflow-hidden max-w-[500px]">{profileBio}</p>
                    </div>
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm font-medium">
                          Level {Math.floor(profileXp / 1000) + 1}
                        </span>
                      </div>
                      <span className="text-xs text-muted-foreground">
                        {profileXp % 1000} / 1000 XP
                      </span>
                    </div>
                    <div className="w-full bg-muted rounded-full h-2.5">
                      <div
                        className="bg-gradient-to-r from-yellow-400 to-yellow-500 h-2.5 rounded-full transition-all duration-500"
                        style={{ width: `${(profileXp % 1000) / 10}%` }}
                      />
                    </div>
                    <p className="text-xs text-muted-foreground mt-2">
                      {1000 - (profileXp % 1000)} XP more to reach Level {Math.floor(profileXp / 1000) + 2}
                    </p>
                  </div>
                </div>
              )}
              ...
              <CardContent className="p-4 md:p-6">
                ...
                <div className="space-y-4 md:space-y-6">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Button variant="outline" size="sm" className="text-xs md:text-sm" onClick={() => navigate('/settings/password')}>
                      Change Password
                    </Button>
                  </div>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {is2FAEnabled ? (
                      <Button variant="destructive" size="sm" className="text-xs md:text-sm" onClick={() => setDisable2FADialogOpen(true)}>
                        Disable 2FA
                      </Button>
                    ) : (
                      <Button variant="outline" size="sm" className="text-xs md:text-sm" onClick={() => navigate('/settings/2fa')}>
                        Enable 2FA
                      </Button>
                    )}
                  </div>
